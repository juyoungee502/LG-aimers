#!/usr/bin/env bash
set -Eeuo pipefail
VERSION="v14"
LOG_FILE="training_${VERSION}.log"
SUBMISSION_FILE="submission_${VERSION}.zip"
RESULT_DIR="outputs"
OOF_FILE="${RESULT_DIR}/${VERSION}_oof_predictions.npz"
WEIGHTED_OOF="${RESULT_DIR}/${VERSION}_weighted_oof_predictions.npz"
RESULT_BUNDLE="${RESULT_DIR}/results_${VERSION}.zip"
cd "$(dirname "$0")"
mkdir -p "$RESULT_DIR"
if [[ ! -f data/train.csv ]]; then
    echo "ERROR: data/train.csv is missing. Run 'git lfs pull' first." >&2
    exit 1
fi
{
    echo "[$(date '+%F %T')] Training weighted component and upgrading to ${VERSION}"
    python train_weighted_component.py --task-type GPU --devices 0
    python upgrade_v13_to_v14.py
    python build_submission.py --output "$SUBMISSION_FILE" \
        --expected-version v14_weighted_catboost
    echo "[$(date '+%F %T')] Training and submission build completed"
} 2>&1 | tee "$LOG_FILE"
python - "$SUBMISSION_FILE" "$LOG_FILE" "$OOF_FILE" "$WEIGHTED_OOF" "$RESULT_BUNDLE" <<'PY'
from pathlib import Path
import sys
import zipfile
inputs = [Path(value) for value in sys.argv[1:5]]
bundle = Path(sys.argv[5])
missing = [str(path) for path in inputs if not path.is_file()]
if missing:
    raise FileNotFoundError(f"Missing result artifacts: {missing}")
with zipfile.ZipFile(bundle, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
    for path in inputs:
        archive.write(path, path.name)
print(f"Result bundle ready: {bundle}")
PY
echo "Copy this file to your PC: ${RESULT_BUNDLE}"
