...
{
    echo "[$(date '+%F %T')] Starting ${VERSION} training on GPU ${GPU_DEVICES}"
    python train_bss_ensemble.py --preset full --task-type GPU \
        --devices "$GPU_DEVICES" --diagnostic-dir "$RESULT_DIR"
