...
{
    "$PYTHON_BIN" research_v55_seed_ensemble.py
    "$PYTHON_BIN" research_v55_v54_scaling.py
    "$PYTHON_BIN" promote_v55_v54_scaling.py
    "$PYTHON_BIN" build_submission.py \
        --expected-version v55_v54_regime_scaling \
        --output submission_v55.zip

    smoke_dir="$(mktemp -d)"
    trap 'rm -rf -- "$smoke_dir"' EXIT
    unzip -q submission_v55.zip -d "$smoke_dir"
...
if len(result) != 5 or not result["control_success"].between(0., 1.).all():
    raise RuntimeError("Packaged v55 smoke test failed")
print("Packaged v55 smoke test passed")
PY
} 2>&1 | tee training_v55.log

"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import zipfile
inputs = [
    Path("submission_v55.zip"), Path("training_v55.log"),
    Path("outputs/v55_oof_predictions.npz"),
    Path("research/v55_seed_ensemble.json"),
    Path("research/v55_v54_scaling.json"),
]
with zipfile.ZipFile("outputs/results_v55.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    for path in inputs:
        archive.write(path, path.name)
print("Created outputs/results_v55.zip")
PY

echo "Copy: outputs/results_v55.zip"
