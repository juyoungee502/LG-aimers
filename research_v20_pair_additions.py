"""Measure pair-table gains after the complete v20 residual portfolio."""
...
def main():
    ...
    old_corrections = {label: apply_frozen_portfolio(frame, old_configs[source])
                       for label, (frame, _y, _base, source) in blocks.items()}
    old_gains = {label: gain(y, base, old_corrections[label])
    ...
    for config in best.values():
        ...
        values = {label: float(config["weight"]) * apply_pair(
        ...
        for scale in (.25, .5, .75, 1., 1.25):
            ...
            for label, (_frame, y, base, _source) in blocks.items():
                total[label] = gain(
                    y, base, old_corrections[label] + scale * values[label]
                )
                marginal[label] = total[label] - old_gains[label]
            audits.append({"name": config["name"], "context": config["context"],
    ...
    output = root / "research/v20_pair_additions.json"
