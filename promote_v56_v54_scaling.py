"""Promote the next public-validated v54 F-scaling step to v56."""
...
def main():
    validation = json.loads(
        (ROOT / "research/v56_v55_step.json").read_text(encoding="utf-8")
    )
    if validation["v56_f_scale"] != F_SCALE:
        raise ValueError("v56 validation and production scale differ")
    ...
    futures = train.loc[
        train["season"].eq(2024), "game_type"
    ].astype(str).eq("F").to_numpy()
    prediction[active] = sigmoid(
        logit(v38) + np.where(futures, F_SCALE, R_SCALE) * (logit(v54) - logit(v38))
    )
    np.savez_compressed(
        ROOT / "outputs/v56_oof_predictions.npz",
    ...
    if "v54_roster_robust_command" not in metadata.get("model_names", []):
        raise ValueError("v56 requires complete v54 model artifacts")
    retired = {
        "v55_v54_regime_scaling", "v60_fraction_confidence",
        "v67_multitask_tabm",
    }
    metadata["model_names"] = [
        name for name in metadata["model_names"] if name not in retired
    ]
    metadata["model_names"].append("v56_v54_regime_scaling")
    for name in retired:
        metadata.pop(name, None)
    metadata["version"] = "v56_v54_regime_scaling"
    metadata["v56_v54_regime_scaling"] = {
        "r_scale": R_SCALE, "f_scale": F_SCALE,
        "anchor": "v54_roster_robust_command",
        "previous_public_version": "v55_v54_regime_scaling",
        "previous_public_score": 1113.6,
        "validation": validation,
        "row_independent_inference": True,
        "forbidden_2025_trackman_used": False,
    }
    metadata_path.write_text(json.dumps(metadata), encoding="utf-8")
    print(json.dumps(metadata["v56_v54_regime_scaling"], indent=2))
