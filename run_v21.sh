...
cd "$(dirname "$0")"
mkdir -p "$RESULT_DIR"
for required in data/train.csv outputs/v19_oof_predictions.npz submit/model/metadata.json; do
    if [[ ! -f "$required" ]]; then
...
{
    echo "[$(date '+%F %T')] Training quarter-robust residual portfolio for ${VERSION}"
    python train_robust_residual_portfolio.py
    python build_submission.py --output "$SUBMISSION_FILE" \
        --expected-version v21_robust_residual_portfolio
