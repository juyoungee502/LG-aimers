"""Train a coherent four-class command-outcome model.

The next within-season cumulative state recovers labels for historical train
rows only.  Success, reverse, middle, and other failure are mutually exclusive,
so a multiclass model avoids the probability-sum inconsistency of three
independent binary failure models.  Evaluation remains chronological.
"""
...
CLASS_NAMES = ("success", "reverse", "middle", "wayoff")
...
def main():
    ...
    overlap = (
        labels["reverse"].fillna(0).to_numpy(float)
        + labels["middle"].fillna(0).to_numpy(float)
    ) > 1.5
    if overlap.any():
        raise ValueError(f"Reverse/middle overlap in {int(overlap.sum())} rows")
    outcome = np.full(len(raw), -1, dtype=np.int8)
    outcome[complete & (target == 1)] = 0
    outcome[complete & (target == 0)
            & labels["reverse"].fillna(0).eq(1).to_numpy()] = 1
    outcome[complete & (target == 0)
            & labels["middle"].fillna(0).eq(1).to_numpy()] = 2
    outcome[complete & (target == 0) & (outcome < 0)] = 3
