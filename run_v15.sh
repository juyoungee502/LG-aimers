...
SPECIALIST_OOF="${RESULT_DIR}/${VERSION}_weighted_categorical_oof_predictions.npz"
...
{
    echo "[$(date '+%F %T')] Training categorical specialist and upgrading to ${VERSION}"
    python train_weighted_categorical_specialist.py --task-type GPU --devices 0
    python upgrade_v14_to_v15.py
    python build_submission.py --output "$SUBMISSION_FILE" \
        --expected-version v15_weighted_categorical_specialist
    echo "[$(date '+%F %T')] Training and submission build completed"
} 2>&1 | tee "$LOG_FILE"
python - "$SUBMISSION_FILE" "$LOG_FILE" "$OOF_FILE" "$SPECIALIST_OOF" "$RESULT_BUNDLE" <<'PY'
