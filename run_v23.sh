...
cd "$(dirname "$0")"
mkdir -p "$RESULT_DIR"
for required in data/train.csv outputs/v19_oof_predictions.npz outputs/v22_oof_predictions.npz submit/model/metadata.json; do
    if [[ ! -f "$required" ]]; then
...
{
    echo "[$(date '+%F %T')] Training probability residual portfolio for ${VERSION}"
    python train_probability_residual_portfolio.py
    python build_submission.py --output "$SUBMISSION_FILE" \
        --expected-version v23_probability_residual_portfolio
