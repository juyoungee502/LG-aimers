...
def add_state_interactions(out: pd.DataFrame) -> pd.DataFrame:
    """Return row-local interactions without repeatedly fragmenting ``out``."""
    ...
    if "pitcher_season_success_s100" in out:
        ...
        features["pitcher_success_x_2strike"] = success * two_strike
        features["pitcher_success_x_advantage"] = success * pitcher_adv
        features["pitcher_success_x_runners"] = success * runners
        features["pitcher_success_x_same_hand"] = success * same_hand
        features["pitcher_success_x_ball_minus_strike"] = success * ball_minus_strike
    for state in ("pitcher_middle", "pitcher_ball",
        ...
        if smooth in out:
            features[f"{state}_x_2strike"] = out[smooth] * two_strike
            features[f"{state}_x_advantage"] = out[smooth] * pitcher_adv
            features[f"{state}_x_runners"] = out[smooth] * runners
            features[f"{state}_x_same_hand"] = out[smooth] * same_hand
            features[f"{state}_x_ball_minus_strike"] = out[smooth] * ball_minus_strike
        if delta in out:
            features[f"{state}_delta_x_2strike"] = out[delta] * two_strike
    ...
    if "pitcher_ball_season_s25" in out:
        features["k2_ball_fast"] = out["pitcher_ball_season_s25"] * two_strike
        features["k2_ball_stable"] = out["pitcher_ball_season_s100"] * two_strike
    if "pitcher_reverse_season_s25" in out:
        features["k2_reverse_fast"] = out["pitcher_reverse_season_s25"] * two_strike
        features["k2_reverse_stable"] = out["pitcher_reverse_season_s100"] * two_strike

    return pd.concat([out, pd.DataFrame(features, index=out.index)], axis=1)
