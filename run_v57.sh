...
{
    # Recreate the public v56 anchor, then audit and freeze only the selected
    # single-table v57 correction.
    "$PYTHON_BIN" research_v56_v54_agreement.py
    "$PYTHON_BIN" research_v56_v55_step.py
    "$PYTHON_BIN" promote_v56_v54_scaling.py
    "$PYTHON_BIN" research_v57_exposure_gate.py
    "$PYTHON_BIN" promote_v57_conservative_r_table.py
    "$PYTHON_BIN" build_submission.py \
        --expected-version v57_conservative_r_table \
        --output submission_v57.zip

    smoke_dir="$(mktemp -d)"
    trap 'rm -rf -- "$smoke_dir"' EXIT
    unzip -q submission_v57.zip -d "$smoke_dir"
...
if len(result) != 5 or not result["control_success"].between(0., 1.).all():
    raise RuntimeError("Packaged v57 smoke test failed")
print("Packaged v57 smoke test passed")
PY
} 2>&1 | tee training_v57.log

"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import zipfile
inputs = [
    Path("submission_v57.zip"), Path("training_v57.log"),
    Path("outputs/v57_oof_predictions.npz"),
    Path("research/v57_exposure_gate.json"),
    Path("research/v57_conservative_r_validation.json"),
]
with zipfile.ZipFile("outputs/results_v57.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    for path in inputs:
        archive.write(path, path.name)
print("Created outputs/results_v57.zip")
PY

echo "Copy: outputs/results_v57.zip"
