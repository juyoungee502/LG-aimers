...
{
    # Recreate the public v56 anchor and the v57 table used only as a frozen
    # direction. V58 then removes v57's positive correction and applies the
    # public-feedback counterstep.
    "$PYTHON_BIN" research_v56_v54_agreement.py
    "$PYTHON_BIN" research_v56_v55_step.py
    "$PYTHON_BIN" promote_v56_v54_scaling.py
    "$PYTHON_BIN" promote_v57_conservative_r_table.py
    "$PYTHON_BIN" research_v58_deploy_curvature.py
    "$PYTHON_BIN" research_v58_public_feedback.py
    "$PYTHON_BIN" promote_v58_public_feedback.py
    "$PYTHON_BIN" build_submission.py \
        --expected-version v58_public_feedback_counterstep \
        --output submission_v58.zip

    smoke_dir="$(mktemp -d)"
    trap 'rm -rf -- "$smoke_dir"' EXIT
    unzip -q submission_v58.zip -d "$smoke_dir"
...
if len(result) != 5 or not result["control_success"].between(0., 1.).all():
    raise RuntimeError("Packaged v58 smoke test failed")
print("Packaged v58 smoke test passed")
PY
} 2>&1 | tee training_v58.log

"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import zipfile
inputs = [
    Path("submission_v58.zip"), Path("training_v58.log"),
    Path("outputs/v58_oof_predictions.npz"),
    Path("research/v58_deploy_curvature.json"),
    Path("research/v58_public_feedback.json"),
]
with zipfile.ZipFile("outputs/results_v58.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    for path in inputs:
        archive.write(path, path.name)
print("Created outputs/results_v58.zip")
PY

echo "Copy: outputs/results_v58.zip"
